...
export default async function handler(req, res) {
  ...
  if (method !== 'PATCH' && method !== 'POST') return res.status(405).json({ detail: 'Method not allowed' });
  const id = (req.query?.id || '').toString();
  const idx = parseInt(req.query?.idx, 10);
  if (!id || Number.isNaN(idx)) return res.status(400).json({ detail: 'Invalid parameters' });
  ...
  if (method === 'PATCH') {
    ...
    const a = answers[idx];
    answers[idx] = { ...a, answer: `${a.answer} (Regenerated)`, is_edited: false };
  }
